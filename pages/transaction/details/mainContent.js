...
import { ContentCard, AccountCard } from "components"
...
const ContentWithoutItems = ({ fromAccount, toAccount }) => {
  return (
    <ContentCard
...
const ContentWithItems = ({ items = [], transactionType }) => {
  ...
  return (
    ...
    >
      {items.map(item => (
        <ItemCard tagColor={color} key={item.name} item={item} />
      ))}
    </ContentCard>
  )
}
