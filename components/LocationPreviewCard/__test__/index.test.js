...
it("gives correct initial states", () => {
  ...
  const wrapper = shallow(<LocationPreviewCardContainer {...mockProps} />)
...
it("prevents malformed location object to be set as state", () => {
  ...
  const spy = jest
    .spyOn(LocationModel, "getLocationById")
    .mockReturnValueOnce(trash)

  const wrapper = mount(<LocationPreviewCardContainer location="whatever" />)
  ...
  expect(jest.isMockFunction(LocationModel.getLocationById)).toBe(true)
